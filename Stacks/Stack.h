...
# define SUPER
	...
	int count(Stack *S, int min, int max){
		...
		while( true ){
			if ( pop(temp) ){
				if ( !(min > *temp->top) && !(max < *temp->top) ){ k++; }
			}
			else{ return k; }
		}
		return k;	
	...
	bool move(Stack* S, Stack* T, int max){
		// Push as many items as possible until max from S onto T
		int k;
		while( move(S, T) ){
